...
# -- URL parsing and provider inference ------------------------------------
...
def test_change_parsing_still_rejects_an_issue_url():
    # The default kind must not quietly accept the wrong sort of URL.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("github.com", "github"),
        ("git.corp.example", None),  # ambiguous: better to ask than to guess
        (None, None),
    ],
)
...
def test_a_configured_host_names_the_forge_of_an_ambiguous_domain(monkeypatch):
    ...
    # An unrelated configured host leaves the domain as ambiguous as it was.
...
def test_github_timeline_yields_the_linked_pull_request(monkeypatch):
    timeline = [
        {"event": "labeled"},
        _cross_reference(11, is_pr=False),  # another issue, not a change
        _cross_reference(12, repo="someone/fork"),  # a fork's PR, not ours
        _cross_reference(13),
    ]
