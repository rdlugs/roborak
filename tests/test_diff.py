...
def test_simple_modification_line_numbers():
    ...
    # Only the log line is added, and it is the 4th line of the new hunk.
...
def test_positions_continue_across_hunks():
    ...
    # First hunk: header=1, then 4 body lines = positions 2..5.
    assert first.line_map == {1: 2, 2: 3, 3: 4, 4: 5}
    assert first.added_lines == {2}

    # Second hunk's header takes position 6, so its body starts at 7.
    assert second.new_start == 21
    assert second.line_map[21] == 7  # "    def run(self):"
    assert second.line_map[22] == 8  # "        data = load()"
    # position 9 is the removed line, which owns no new-file line number
    assert second.line_map[23] == 10  # the added "return validate(data)"
...
def test_multiple_files_in_one_diff():
    ...
    # Positions restart per file, since forges scope them per file.
...
def test_trimming_preserves_line_numbers():
    ...
    # The surviving numbers must still be the true new-file numbers: line 30 is the
    # insertion, so the context line after it is new-file line 31.
...
def test_trimming_keeps_context_around_every_change():
    ...
    # Two separated change regions leave a gap, which must be marked so the model
    # does not read the jump in line numbers as contiguous code.
