...
> selected profile > defaults. Select one profile with CLI > `ROBORAK_PROFILE` > project > user >
`balanced`; explicit fields always win over preset defaults. Verification resolves the profile
from its trusted base configuration, never the working tree. `src/roborak/config_template.yaml`
is the annotated source of truth for every key and default, mirrored by `core/config.py`. `.roborak.yaml` is the canonical generated default and wins
