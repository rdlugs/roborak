...
def strip_fences(text: str) -> str:
    ...
    # A reply may open a fence and never close it.
    if text.startswith("```"):
        body = text.split("\n", 1)[1] if "\n" in text else ""
...
def _salvage_prefix(text: str) -> Any:
    ...
    for cut in range(len(lines) - 1, 0, -1):
        # Only try cutting at the start of a list item, so we never keep half a finding.
...
def _coerce_finding(entry: dict[str, Any], valid_files: set[str] | None) -> Finding | None:
    ...
    if valid_files is not None and path not in valid_files:
        # The model named a file that is not in the diff -- a hallucinated anchor.
...
def _as_int(value: Any) -> int | None:
    ...
    if isinstance(value, int):
        return value
    try:
        # Models sometimes answer "42" or even "line 42".
...
def _as_confidence(value: Any) -> float:
    try:
        ...
        return 0.8
    if number > 1.0:  # a model answering "85" rather than "0.85"
        number /= 100.0
...
def _clean_suggestion(value: Any) -> str | None:
    ...
    # Models occasionally leave diff markers in a replacement block.
    lines = text.split("\n")
