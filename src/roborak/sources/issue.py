...
class GitLabIssueSource:
    ...
    def load(self) -> Issue:
        ...
        return _build_issue(
            ...
            # `system` notes are label changes and milestone edits -- noise, not
            # requirements.
    ...
    def linked_changes(self) -> list[LinkedChange]:
        ...
        for entry in related:
            ...
            if not isinstance(iid, int):
                continue
            # A merge request in another project cannot be reviewed with this
            # target, and guessing at its project path would be worse than saying
            # nothing.
...
class GitHubIssueSource:
    ...
    def load(self) -> Issue:
        ...
        if payload.get("pull_request"):
            # GitHub serves pull requests from the issues endpoint too; reviewing
            # one as an issue would silently produce nonsense.
            raise SourceError(
