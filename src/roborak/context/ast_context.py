...
# tree-sitter grammar names, where they differ from our language names.
_GRAMMAR = {
    "csharp": "csharp",
    "cpp": "cpp",
    "shell": "bash",
...
# Node types that count as "a symbol worth showing whole". Deliberately a flat
# set spanning every grammar: several languages share a node name, and matching
# by name across all of them is both correct and cheaper than a per-language map.
...
def enclosing_symbol(file: ChangedFile, hunk: Hunk) -> SymbolSpan | None:
    ...
    # tree-sitter rows are 0-based; our line numbers are 1-based.
    ...
    for node in _walk(root):
        # Skip the root: Python's root node is itself typed `module`, which is
        # also Ruby's `module` keyword, so matching it would return the whole file.
        ...
        # Smallest containing symbol wins: the function, not its class.
